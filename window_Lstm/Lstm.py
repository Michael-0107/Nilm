...
class WindowGRUDisaggregator(Disaggregator):
    ...
    def _create_model(self):
        ...
        # 1D Conv
        model.add(tf.keras.layers.Conv1D(16, 4, activation='tanh', input_shape=(self.window_size,1), padding="same", strides=1))

        #Bi-directional GRUs
        model.add(tf.keras.layers.Bidirectional(tf.keras.layers.GRU(64, activation='tanh', return_sequences=True), merge_mode='concat'))
        model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Bidirectional(tf.keras.layers.GRU(128, activation='tanh', return_sequences=False), merge_mode='concat'))
        model.add(tf.keras.layers.Dropout(0.5))
